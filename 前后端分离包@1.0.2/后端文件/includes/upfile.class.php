...
class upfiles{
    ...
    //上传方法
    public function upload($db,$user_id,$fileCheck = false){
        ...
        if(!empty($this->allowType)){
            if(!is_integer(strpos($this->allowType, $_FILES[$this->fileField]['type']))){
                $this->error = '上传文件类型错误(禁止'.$_FILES[$this->fileField]['type'].'类型)';
                return false;
            }
        }
        ...
        if(!is_integer(strpos($this->allowExeName, $this->exeName))){
            $this->error = '上传文件扩展名错误(禁止['.$this->exeName.']后缀)';
            return false;
        }
        ...
        //保留原生文件名
        $filename_n = $_FILES[$this->fileField]['name'];
        ...
        if(is_null($this->uploadedFileUseName)) {
            ...
            switch ($this->renameRule){
                case 1:
                    ...
                    break;
                case 2:
                    $this->fileName = mb_strtoupper(uniqid(more_entropy: true)).'.'.$this->exeName;
                    $this->targetDir .= '/'.$this->fileName;
                    break;
                case 3:
        ...
        if(move_uploaded_file($_FILES[$this->fileField]['tmp_name'], $this->targetDir)){
            ...
            #文件路径
            $file_path = str_replace('../../..', '', $this->uploadedFileUrl);
            $file_path = str_replace('../..', '', $file_path);
            #计算文件大小
            $file_size = $_FILES[$this->fileField]['size'];
            if($file_size<=0.00){
                $file_size = 0.01;
            }
            #文件指纹
            $file_md5 = md5($this->fileName);
            #文件下载链接
            $durl = $url.'/f/'.$file_md5;
            $db->insert('file_record',['create_user_id'=>$user_id,'name'=>$filename_n,'fmd5'=>$file_md5,'content_type'=>$this->exeName,'length'=>$file_size,'path'=>$file_path,'durl'=>$durl]);
            return $file_path;
        }
        $this->error = '文件上传失败';
        return false;
