...
#检测访问权限
@checkPower('agent:subordinate:save');
...
#检查账户是否有权限添加子代
if(!$user_info['open_agent'])exJson(1,'暂无添加子代权限');

#检查账户是否已存在
$row = $db->find('user','*',['username'=>$arr['username'],'create_user_id'=>$user_info['create_user_id']]);
if($row){
...
$saveData['agent_id'] = $user_info['user_id'];
